...
export async function POST(req) {
  ...
  const sessionToken = await userController.generateSessionToken(
    registerAttempt.uuid
  );
  console.log(
    `${email} has successfully registered, generated a new sessionToken: ${sessionToken.tokenID}`
  );
  ...
  res.cookies.set("displayName", email);
